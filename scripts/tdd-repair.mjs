 ...
 *   - Conformant mode (`--no-test-oracle`) — would need repro generation from
 *     scratch instead of a pre-existing failing test. Falls back to
 *     "test file required" for now.
 *
 * USAGE
  ...
  if (ARGS.noTestOracle) {
    emit({
      ...
      data: {
        ...
        hint: 'Conformant mode (`--no-test-oracle`) is not implemented — it would need repro generation from scratch. For now, write a failing test first and use the default Test-Driven mode.',
      },
    }, 2);
  }
