// similarity.mjs — weighted similarity module.
//
// Adapted from ruflo (github.com/ruvnet/ruflo), MIT.
//
// DESIGN CONSTRAINTS PRESERVED
//   Removable ✓  pure-JS — no external import path
//   Optional  ✓  no new dependency
//   Graceful  ✓  malformed input → low-confidence output, never throws
//   Standalone ✓  unit-importable; no CLI or MCP required
//
// CONTRACT (PUBLIC)
...
//
// The opts.weights hook lets consumers experiment with per-org weight
// overrides without forking the module — it is NOT a stable public API.
...
// ─────────────────────────────────────────────────────────────────────
// 9-dim feature vector. Missing fields default to 0 — that's the
...
function perDimensionBreakdown(a, b, va, vb, weights) {
  ...
  // why two items scored as they did.
  const out = {};
